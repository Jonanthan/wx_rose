...
static  berr hijack_rule_match(hytag_t *hytag, hijack_rule_t **rule, uint8_t *hijack_flag)
{
    ...
    for ( i = 0; i < HIJACK_RULE_NUM_MAX; i++)
    {
		...
		else if(HIJACK_WX_MODE == ptr[i].hijack.mode)
		{
		    //  dip  match
			if((hytag->outer_dstip4 & ptr[i].hijack.dip_mask)  !=  (ptr[i].hijack.dip & ptr[i].hijack.dip_mask))
			{
			     continue;
			}

            //     dml_index  > 0 :  dml
            if(ptr[i].hijack.dml_index > 0)
            {
                if(! dml_exist(ptr[i].hijack.dml_index, hytag->host))
                {
                    continue;
                }

                //  只保留直接访问
                if(strcmp(hytag->referer, "") != 0)
                {
                //    continue;
                }
                ...
                //  http://host/
                snprintf(tmp_buf, sizeof(tmp_buf), "%s/", hytag->host);
                if(strcmp(tmp_buf, hytag->url) == 0)
                {
                    *hijack_flag = HIJACK_WX_MODE;
                    goto hjk_wx_match;
                }                 

                //  http://host/?
                snprintf(tmp_buf, sizeof(tmp_buf), "%s/?", hytag->host);
                if(strncmp(tmp_buf, hytag->url, strlen(tmp_buf)) == 0)
                {
                    *hijack_flag = HIJACK_WX_MODE;
                    goto hjk_wx_match;
                }
                ...
                //  http://host/index
                snprintf(tmp_buf, sizeof(tmp_buf), "%s/index", hytag->host);
                if(strncmp(tmp_buf, hytag->url, strlen(tmp_buf)) == 0)
                {
                    *hijack_flag = HIJACK_WX_MODE;
                    goto hjk_wx_match;
...
hjk_wx_match:
            ...
            {
                //  prce_url
                compare  = pcre_exec( ptr[i].hijack.cre,
...
berr naga_hijack(hytag_t *hytag)
{
	...
	else if(HIJACK_WX_MODE == rule->mode)
	{
        snprintf(hijack_url, 1024, "http://%s", rule->key);
	}
    else
    ...
    if(0)
    {
        FILE *fp = fopen("./hjk.txt", "a+");
        fprintf(fp, "old url is : %s.\n", hytag->url);
        fprintf(fp, "new url is : %s.\n", hijack_url);
        fclose(fp);
    }
