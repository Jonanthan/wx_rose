...
static int hijack_cmd_get(struct vty *vty, const char *index_str)
{
    ...
    if (HIJACK_RULE_EFFECTIVE == effect)
    {
		...
		else  if(hijack.mode == HIJACK_WX_MODE)
		{			
			struct in_addr netmask;
			netmask.s_addr = htonl(hijack.dip_mask);
		
			vty_out(vty, "%-8d%-5s%d.%d.%d.%d/%-3d%-54s%-54s%-12ld%-12ld%s",
				hijack.index,
				"wx",
				(hijack.dip >> 24) & 0xff,
				(hijack.dip >> 16) & 0xff,
				...
				hijack.prce_url,
				hijack.key,
				(uint64_t)hijack.acl.cnt.cnt, (uint64_t)hijack.acl.pushed_cnt.cnt, VTY_NEWLINE);
		}
		else
		{
    ...
    for (i = 0; i < HIJACK_RULE_NUM_MAX; i++)
    {
        ...
        if (HIJACK_RULE_EFFECTIVE == effect)
    	{
			...
			{			
				...
				vty_out(vty, "%-8d %-5s %d.%d.%d.%d/%-3d %-5d %-54s %-54s %-12ld %-12ld %s",
					hijack.index,
					"wx",
					(hijack.dip >> 24) & 0xff,
					(hijack.dip >> 16) & 0xff,
...
static int hijack_cmd_wx_add(struct vty *vty, const char *index_str, const char *mode, const char*ip_str,  const char * dml_index_str, const char *pcre_url, const char *key)
{
    int ret = 0, i;
    uint32_t index = 0;
    char key_str[MAX_HIJACK_STR];
    ...
    if(!strcmp("wx", mode))
    {
        hijack.mode = HIJACK_WX_MODE;
    }
	else
	{
        vty_out(vty, "Error: mode not wx! %s", VTY_NEWLINE);
        return CMD_WARNING;
	}
    ...
    hijack.dip = ip;
	hijack.dip_mask = ntohl(mask.s_addr);



    // dml_index_str 
    // 0:not care 
    hijack.dml_index = atoi(dml_index_str);
    ...
    {        
        switch(pcre_url[i])
        {
            case 'Q':
                ...
                break;
            default:
                url_str[url_chr_index++] = pcre_url[i];
                break;
        }
        lastchar = pcre_url[i];
    }
    url_str[url_chr_index]  = '\0';     
    memcpy(hijack.prce_url, url_str, strlen(url_str));
...
DEFUN(hjk_set_wx,
      hjk_set_wx_cmd,
      "hjkwx   add   <0-49>   A.B.C.D/M   DML_INDEX   PCRE_URL   NEW_URL",
      HIJACK_STR
      ADD_STR
      HIJACK_INDEX
	  "Dest ip/mask \n"
...
{
    return hijack_cmd_wx_add(vty, argv[0], "wx", argv[1], argv[2], argv[3], argv[4]);
}
...
void hijack_cmd_config_write(struct vty *vty)
{
    ...
    for (i = 0; i < HIJACK_RULE_NUM_MAX; i++) 
    {
        ...
        if (HIJACK_RULE_EFFECTIVE == effect)
    	{
            ...
            else if(HIJACK_WX_MODE == hijack.mode)
            {
				struct in_addr netmask;
				netmask.s_addr = htonl(hijack.dip_mask);
			
				vty_out(vty, "hjkwx  add  %d  %d.%d.%d.%d/%d  %d  %s  %s%s",
					hijack.index,
					(hijack.dip >> 24) & 0xff,
					(hijack.dip >> 16) & 0xff,
					(hijack.dip >>	8) & 0xff,
...
void cmdline_hijack_init(void)
{
    ...
    install_element(CMD_NODE, &hjk_set_wx_cmd);
